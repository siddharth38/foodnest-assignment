...
const OrderSummry = () => {
  ...
  const [showModal,setShowModal]=useState(error?true:false)
    ...
    const handlePlaceOrder =()=>{
       dispatch(placeOrder({orderItems:cartItems,shippingAddress:address,paymentMethod:paymentMethod,itemsPrice:cartPrice,totalprice:totalPrice,shippingPrice:deleviryPrice,email:user.eamil,userName:user.name ,_id :user._id}))
    }

    const PaymentHandler=(paymentResult)=>{
      const {razorpay_payment_id}=paymentResult
      dispatch(placeOrder({orderItems:cartItems,shippingAddress:address,paymentMethod:paymentMethod,itemsPrice:cartPrice,totalprice:totalPrice,shippingPrice:deleviryPrice,email:user.eamil,_id :user._id,userName:user.name,paymentId:razorpay_payment_id}))
 ...
 const paymentObject = new window.Razorpay(options);
  ...
  const getPaymentButton=()=>{
    if(paymentMethod==='COD'){
     return <button onClick={handlePlaceOrder} disabled={totalPrice===0?true:false}>{loading?"loading":'PLACE ORDER'}</button>
